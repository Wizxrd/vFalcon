...
{
    public async static Task<Dictionary<string, List<ProcessedFeature>>> GetAllStarsFeatures()
    {
        JArray starsVideoMapIds = new();
        foreach (string id in Artcc.GetFacilityIds())
        {
            List<VideoMap> videoMaps = VideoMap.GetChildFacilityVideoMaps(id);
            foreach (VideoMap videoMap in videoMaps)
            {
                starsVideoMapIds.Add(videoMap.id);
            }
        }
        Dictionary<string, List<ProcessedFeature>> starsFeatures = await GetStarsFacilityFeatures(App.Artcc, starsVideoMapIds);
        return starsFeatures;
    }

    public async static Task<Dictionary<string, List<ProcessedFeature>>> GetEramFeatures()
    {
        JArray eramVideoMapIds = new();
        if (App.Profile.MapSettings.GeoMap != null)
        {
            List<string> mapsetVideoMapIds = VideoMap.GetMapsetVideoMapIds(App.Profile.MapSettings.GeoMap);
            foreach (string id in mapsetVideoMapIds)
            {
                eramVideoMapIds.Add(id);
            }
        }
        Dictionary<string, List<ProcessedFeature>> eramFeatures = await GetEramFacilityFeatures(App.Artcc, eramVideoMapIds);
        return eramFeatures;
    }

    public static async Task<Dictionary<string, Dictionary<string, List<ProcessedFeature>>>> GetAllEramFeatures()
    {
        Dictionary<string, Dictionary<string, List<ProcessedFeature>>> combined = new();
        JArray eramVideoMapIds = new();
        foreach (JObject geoMap in VideoMap.GetGeoMaps())
        {
            JArray videoMapIds = (JArray)geoMap["videoMapIds"];
            foreach (string id in videoMapIds)
            {
                eramVideoMapIds.Add(id);
            }
            Dictionary<string, List<ProcessedFeature>> eramFeatures = await GetEramFacilityFeatures(App.Artcc, eramVideoMapIds);
            combined[(string)geoMap["name"]] = eramFeatures;
            eramVideoMapIds.Clear();
        }
        return combined;
    }

    public static async Task<Dictionary<string, List<ProcessedFeature>>> GetEramFacilityFeatures(Artcc artcc, JArray activeVideoMapIds)
    {
        try
        {
            Logger.Info("GeoMap.LoadFacilityFeatures", "Creating FacilityFeatures...");
            string sourcePath = Path.Combine(PathFinder.GetAppDirectory(), $"VideoMaps\\{artcc.id}");
    ...
    public static async Task<Dictionary<string, List<ProcessedFeature>>> GetStarsFacilityFeatures(Artcc artcc, JArray activeVideoMapIds)
    {
        try
        {
